### THIS IS VIBECODED DOGSHIT BUT IT WORKS
...
import modal
from app.models.vlm_models import VLMResponseFormat, VLMOutput, DataPoint
...
async def run_modal_inference(img_bytes: bytes, filename: str = "") -> VLMResponseFormat:
    ...
    try:
        ...
        # Get prompt from gateway's prompts directory
        prompt = None
        try:
            ...
            if os.path.exists(prompt_path):
                with open(prompt_path, "r", encoding="UTF-8") as f:
                    prompt = f.read().strip()
                logger.debug(f"Loaded default prompt from {prompt_path}")
        except Exception as e:
            logger.warning(f"Could not load default prompt: {e}")
        ...
        try:
            if isinstance(result, list) and result:
                ...
                if hasattr(first, "model_dump"):
                    ...
                    json_str = json.dumps(first.dict(exclude_none=True))
                elif isinstance(first, dict):
                    json_str = json.dumps(first)
                elif hasattr(first, "model_dump_json"):
                ...
                try:
                    json.loads(result)
                    return VLMResponseFormat(json_data=result, inference_time_ms=processed_time)
                except:
            ...
            logger.error(f"Error processing Modal result: {e}")
            return VLMResponseFormat(text=str(result), inference_time_ms=processed_time)
    except Exception as e:
